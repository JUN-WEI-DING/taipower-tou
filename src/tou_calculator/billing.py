...
from tou_calculator.custom import build_tariff_plan, build_tariff_profile, build_tariff_rate
...
def calculate_bill(
    ...
    calendar = calendar_instance or taiwan_calendar(cache_dir=cache_dir, api_timeout=api_timeout)
    ...
    billing_periods = _billing_period_index(usage_for_billing.index, inputs.billing_cycle_months)
...
def calculate_bill_breakdown(
    ...
    period_usage.index = period_usage.index.set_names(["period", "season", "period_type"])
...
def _build_tariff_plan_from_data(
    ...
    if tiered and billing_cycle_months and billing_cycle_months > 1:
        ...
        for item in tiered:
            ...
            entry["max"] = None if entry["max"] is None else entry["max"] * billing_cycle_months
            scaled.append(entry)
        tiered = scaled
...
def _season_strategy(plan_data: dict[str, Any], store: PlanStore) -> TaiwanSeasonStrategy:
...
def _calculate_basic_fees(
    ...
    if formula and inputs.contract_capacities:
        monthly = _basic_fee_from_formula(plan_data, inputs, month_index, store, formula)
    else:
        ...
        for entry in plan_data.get("basic_fees", []):
            ...
            for idx, season_label in enumerate(season_labels):
                if "summer" in entry or "non_summer" in entry:
                    rate = entry.get("summer") if season_label == "summer" else entry.get(
                        "non_summer"
                    )
                else:
                    rate = entry.get("cost")
...
def _calculate_surcharges(
    ...
    rules = plan_data.get("billing_rules", {})
    rule = rules.get("over_2000_kwh_surcharge") or plan_data.get("over_2000_kwh_surcharge")
...
def _calculate_adjustments(
    ...
    if pf_rule and inputs.power_factor is not None:
        ...
        if pf < base:
            delta = base - pf
            adjustment += target * (delta * step / 100.0)
        elif pf > base:
        ...
        if base_rate is not None:
            ...
            if over_series is not None:
                ...
                for idx, over in over_series.items():
                    contract_kw = inputs.contract_capacity_kw or inputs.contract_capacities.get(
                        "regular", 0.0
                    )
                    threshold = contract_kw * threshold_ratio
                    over_low = min(over, threshold)
                    over_high = max(0.0, over - threshold)
                    adjustment.loc[idx] += base_rate.loc[idx] * over_low * rate_low
                    adjustment.loc[idx] += base_rate.loc[idx] * over_high * rate_high
...
def _basic_fee_rate_for_label(
    ...
    for season_label in season_labels:
        if "summer" in entry or "non_summer" in entry:
            rate = entry.get("summer") if season_label == "summer" else entry.get("non_summer")
        else:
            rate = entry.get("cost")
...
def _basic_fee_from_formula(
    ...
    for idx, season_label in enumerate(season_labels):
        if formula["type"] == "regular_only":
            ...
            monthly.iloc[idx] += rate * capacities.get("regular", 0.0)
            continue
        ...
        if formula["type"] == "two_stage":
            ...
            weekend_base = (saturday + off_peak) - (regular + non_summer) * weekend_ratio
            weekend_base = max(0.0, weekend_base)

            if season_label == "summer":
                monthly.iloc[idx] += regular_rate * regular
                monthly.iloc[idx] += saturday_rate * weekend_base
            else:
                monthly.iloc[idx] += regular_rate * regular
                monthly.iloc[idx] += non_summer_rate * non_summer
                monthly.iloc[idx] += saturday_rate * weekend_base
            continue
        ...
        if formula["type"] == "three_stage":
            ...
            monthly.iloc[idx] += regular_rate * regular
            monthly.iloc[idx] += semi_rate * semi_peak
            monthly.iloc[idx] += saturday_rate * weekend_base
...
def _apply_minimum_usage(
    ...
    if not ref:
        return usage
    if inputs.meter_phase is None or inputs.meter_voltage_v is None or inputs.meter_ampere is None:
        return usage
...
def _billing_period_index(index: pd.DatetimeIndex, cycle_months: int | None) -> pd.PeriodIndex:
...
def _compute_over_contract_kw(
    ...
    if inputs.over_contract_kw is not None:
        return pd.Series(inputs.over_contract_kw, index=billing_periods.unique().to_timestamp())
    demand = inputs.demand_kw
    if demand is None:
        return None
    if not isinstance(demand, pd.Series) or not isinstance(demand.index, pd.DatetimeIndex):
    ...
    for period, group in max_by_cat.groupby(level=0, sort=False):
        ...
        over = _calculate_over_contract_from_categories(values, inputs.contract_capacities, oc_rule)
...
def _calculate_over_contract_from_categories(
    ...
    if tier == "three_stage":
        ...
        saturday_over = max(
            0.0, max_demand.get("saturday_semi_peak", 0.0) - (regular + semi_peak + saturday)
        )
        off_over = max(
            0.0,
            max_demand.get("off_peak", 0.0) - (regular + semi_peak + saturday + off_peak),
        )
    ...
    saturday_over = max(
        0.0, max_demand.get("saturday_semi_peak", 0.0) - (regular + non_summer + saturday)
    )
    off_over = max(
        0.0,
...
def _tiered_total_cost(
    ...
    for tier in sorted_tiers:
        ...
        unit_cost = tier.summer_cost if season_label == "summer" else tier.non_summer_cost
...
def _calculate_period_costs(
    ...
    for period, group in usage.groupby(billing_periods):
        ...
        if rates.tiered_rates:
            ...
            season_label = season.value if hasattr(season, "value") else str(season)
            total_cost = _tiered_total_cost(month_usage.sum(), season_label, rates.tiered_rates)
            records.append(
                {
        ...
        unit_costs = pd.Series(
            [
                ...
                for season, period_type in zip(month_context["season"], month_context["period"])
            ],
            index=period_index,
        )
        costs = month_usage * unit_costs
        grouped = costs.groupby([month_context["season"], month_context["period"]]).sum()
        for (season, period_type), cost in grouped.items():
            season_label = season.value if hasattr(season, "value") else str(season)
            period_label = period_type.value if hasattr(period_type, "value") else str(period_type)
            records.append(
                {
                    "period": period.to_timestamp(),
                    "season": season_label,
